...
public final class PackedCard {
    ...
    /**
     * Check si une carte est valide ou pas (si les bits pour représenter le
     * rang correspondent à un rang et si les bits inutilisés sont tous des 0)
     * 
     * @param pkCard
     *            (int) carte sous forme int (séquence de 32 bits)
    ...
    public static boolean isValid(int pkCard) {
        return Bits32.extract(pkCard, BIT_START, CARD_RANK_BITS) <= RANK_MAX_VALUE
                && Bits32.extract(pkCard, CARD_BITS,
                        Integer.SIZE - CARD_BITS) == VALUE_ZERO;
    }

    /**
     * Réunit une couleur et un rang en une carte et retourne la séquence de
     * bits qui caractérise la carte
     * 
     * @param c
    ...
    public static int pack(Card.Color c, Card.Rank r) {
        return Bits32.pack(r.ordinal(), CARD_RANK_BITS, c.ordinal(),
                CARD_COLOR_BITS);
    }
     ...
     * @param pkCard
     *            (int) Carte sous forme int (séquence de 32 bits)
     * @return (Card.Color) la couleur de la carte, null si appartient à aucun
     *         cas
     */
    public static Card.Color color(int pkCard) {
        assert isValid(pkCard);
        return Card.Color.ALL
                .get(Bits32.extract(pkCard, CARD_RANK_BITS, CARD_COLOR_BITS));
     ...
     * 
     * @param pkCard
     *            (int) Une carte sous forme int (séquence de 32 bits)
     * @return (Card.Rank) le rang de la carte null, si appartient à aucun cas
     */
    public static Card.Rank rank(int pkCard) {
        assert isValid(pkCard);
        return Card.Rank.ALL
                .get(Bits32.extract(pkCard, BIT_START, CARD_RANK_BITS));
    }

    /**
     * Retourne vrai ssi la première carte donnée est supérieure à la seconde,
     * sachant que l'atout est trump ATTENTION: cette méthode retourne faux si
     * les deux cartes ne sont pas comparables
     * 
     * @param trump
     *            (Card.Color) l'atout du jeu
     * @param pkCardL
     *            (int) carte sous forme de int (empaquetée), la première carte
     * @param pkCardR
     *            (int) carte sous forme de int (empaquetée), la seconde carte
    ...
    public static boolean isBetter(Card.Color trump, int pkCardL, int pkCardR) {
        ...
        if (!color(pkCardL).equals(trump) && !color(pkCardR).equals(trump)
                && color(pkCardL).equals(color(pkCardR))
     ...
     *            (Card.Color) l'atout du jeu
     * @param pkCard
     *            (int) carte sous forme de int (empaquetée)
