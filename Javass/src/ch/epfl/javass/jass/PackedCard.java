...
public final class PackedCard {
	...
	/**
	 * NE représente PAS une carte empaquetée valide
	 */
	...
	/**
	 * Check si une carte est valide ou pas (si les bits pour représenter le rang
	 * correspondent à un rang et si les bits inutilisés sont tous des 0)
	 * 
	 * @param pkCard (int) carte sous forme int (séquence de 32 bits)
	 * @return (boolean) la valeur de la validité d'une carte
	 */
	public static boolean isValid(int pkCard) {
		return Bits32.extract(pkCard, BIT_START, CARD_RANK_BITS) <= RANK_MAX_VALUE
				&& Bits32.extract(pkCard, CARD_BITS, Integer.SIZE - CARD_BITS) == VALUE_ZERO;
	}

	/**
	 * Réunit une couleur et un rang en une carte et retourne la séquence de bits
	 * qui caractérise la carte
	 * 
	 ...
	 * 
	 * @param pkCard (int) Carte sous forme int (séquence de 32 bits)
	 * @return (Card.Color) la couleur de la carte, null si appartient à aucun cas
	 */
	 ...
	 * 
	 * @param pkCard (int) Une carte sous forme int (séquence de 32 bits)
	 * @return (Card.Rank) le rang de la carte null, si appartient à aucun cas
	 */
	...
	/**
	 * Retourne vrai ssi la première carte donnée est supérieure à la seconde,
	 * sachant que l'atout est trump ATTENTION: cette méthode retourne faux si les
	 * deux cartes ne sont pas comparables
	 * 
	 * @param trump   (Card.Color) l'atout du jeu
	 * @param pkCardL (int) carte sous forme de int (empaquetée), la première carte
	 * @param pkCardR (int) carte sous forme de int (empaquetée), la seconde carte
	...
	public static boolean isBetter(Card.Color trump, int pkCardL, int pkCardR) {
		...
		if (!color(pkCardL).equals(trump) && !color(pkCardR).equals(trump) && color(pkCardL).equals(color(pkCardR))
	...
	/**
	 * Retourne la valeur de la carte empaquetée donnée en argument, sachant que
	 * l'atout est trump
	 * 
	 * @param trump  (Card.Color) l'atout du jeu
	 * @param pkCard (int) carte sous forme de int (empaquetée)
	 ...
	 * chaîne de caractères composée du symbole de la couleur et du nom abrégé du
	 * rang
	 * 
	 * @param pkCard (int) carte sous forme de int (empaquetée)
