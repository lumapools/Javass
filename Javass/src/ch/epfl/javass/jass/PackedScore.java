...
public final class PackedScore {
	...
	/**
	 * Check si un score est valide ou pas (si les bits pour représenter le nb
	 * correspondent à un nb de plis valide, même chose pour les points du tour et
	 * même chose pour les points totaux et si les bits inutilisés sont tous des 0),
	 * et cela pour les 2 équipes
	 * 
	 * @param pkScore (long) score sous forme long (séquence de 64 bits)
	 * @return (boolean) la valeur de la validité d'un score
	 */
	public static boolean isValid(long pkScore) {
		return !(Bits64.extract(pkScore, TRICK_BITS_START_T1, NB_TRICK_BITS) > MAX_TRICKS_PER_TURN
	 ...
	 *                    courant
	 * @param gamePoints1 (int) nb de points remportés par l'équipe 1 au total
	 * @param turnTricks2 (int) nb de plis remportés par l'équipe 2
	 * @param turnPoints2 (int) nb de points remportée par l'équipe 2 dans le tour
	 *                    courant
	 * @param gamePoints2 (int) nb de points remportés par l'équipe 2 au total
	...
	public static long pack(int turnTricks1, int turnPoints1, int gamePoints1, int turnTricks2, int turnPoints2,
			int gamePoints2) {
		long pkScore1 = (long) Bits32.pack(turnTricks1, NB_TRICK_BITS, turnPoints1, NB_TURN_POINTS_BITS, gamePoints1,
				NB_GAME_POINTS_BITS);
		long pkScore2 = (long) Bits32.pack(turnTricks2, NB_TRICK_BITS, turnPoints2, NB_TURN_POINTS_BITS, gamePoints2,
				NB_GAME_POINTS_BITS);
	 ...
	 * @param t(TeamId) l'équipe de laquelle on veut le nombre de plis remportés
	 ...
	 * 
	 * @param pkScore(long) score empaqueté
	 * @param t(TeamId) l'équipe de laquelle on veut les points remportés
	...
	/**
	 * Retourne le nombre de points remportés par l'équipe donnée dans les tours
	...
	/**
	 * Retourne le nombre total des points remportés par l'équipe donnée dans la
	 * partie courante des scores empaquetés donnés
	 * 
	 * @param pkScore(long) le score empaqueté
	 * @param t(TeamId) l'équipe de laquelle on veut le nombre de points total
	...
	/**
	 * Met à jour les scores de la partie (sans mettre le score total de l'équipe à
	 * jour), et si le nombre de plis gagnés est égal à 9, alors on ajoute le nb de
	 * points additionels (MATCH_ADDITIONAL_POINTS) aux points du tour de l'équipe
	 * qui a fait match
	 * 
	 * @param pkScore     (long) le score actuel empaqueté sous forme de séquence de
	 *                    64 bits
	 * @param winningTeam (TeamId) l'équipe qui gagne le pli
	 * @param trickPoints (int) les points gagnés durant ce tour
	 * @return (long) le score actuel empaqueté sous forme de séquence de 64 bits
	 *         (sans avoir mis à jour le score total des 2 équipes)
	 */
	public static long withAdditionalTrick(long pkScore, TeamId winningTeam, int trickPoints) {
		...
		if (tricksUpdated == Jass.TRICKS_PER_TURN) {
			scoreUpdated = turnPoints(pkScore, winningTeam) + trickPoints + Jass.MATCH_ADDITIONAL_POINTS;
		} else {
			scoreUpdated = turnPoints(pkScore, winningTeam) + trickPoints;
		}
		if (winningTeam.equals(TeamId.TEAM_1)) {
			return pack(tricksUpdated, scoreUpdated, gamePoints(pkScore, TeamId.TEAM_1),
		...
		return pack(turnTricks(pkScore, TeamId.TEAM_1), turnPoints(pkScore, TeamId.TEAM_1),
				gamePoints(pkScore, TeamId.TEAM_1), tricksUpdated, scoreUpdated, gamePoints(pkScore, TeamId.TEAM_2));
	}
	...
	public static long nextTurn(long pkScore) {
		assert isValid(pkScore);
		return pack(INITIAL_NB_TRICKS, INITIAL_TURN_POINTS, totalPoints(pkScore, TeamId.TEAM_1), INITIAL_NB_TRICKS,
				INITIAL_TURN_POINTS, totalPoints(pkScore, TeamId.TEAM_2));
	}

	/**
	 * Représentation textuelle des scores
	 * 
	 * @param pkScore(long) score empaquetée
	...
	public static String toString(long pkScore) {
		return "(" + turnTricks(pkScore, TeamId.TEAM_1) + "," + turnPoints(pkScore, TeamId.TEAM_1) + ","
				+ gamePoints(pkScore, TeamId.TEAM_1) + "," + totalPoints(pkScore, TeamId.TEAM_1) + ")/("
				+ turnTricks(pkScore, TeamId.TEAM_2) + "," + turnPoints(pkScore, TeamId.TEAM_2) + ","
				+ gamePoints(pkScore, TeamId.TEAM_2) + "," + totalPoints(pkScore, TeamId.TEAM_2) + ")";
	}
}