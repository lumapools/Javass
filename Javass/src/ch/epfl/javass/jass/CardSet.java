...
public final class CardSet {
     ...
     * Constructeur privé
     * @param packed (long)
     *          Séquence de 64 bits qui représente la version empaquetée de la carte
     */
     ...
     * @param cards (List contenant des Card)
     *          La liste de cartes dont on veut créer un CardSet
     * @return (CardSet) obtenu à partir de la liste des cartes
     */
    public static CardSet of(List<Card> cards) {
        long cardSet = EMPTY.packed();
        for(Card c : cards) {
     ...
     * @param packed (long)
     *          le cardSet sous forme de séquence de 64 bits
     ...
     * @param index (int)
     *          l'index auquel on veut la carte
     ...
     * 
     * @param card (Card)
     *          la carte qu'on veut ajouter à l'ensemble
     ...
     * 
     * @param card (Card)
     *          la carte à supprimer de l'ensemble
     ...
     * @param card (Card)
     *          la carte qu'on veut vérifier
     * @return (boolean) vrai si la carte donnée en argument est contenue dans le cardSet, sinon faux
     */
    ...
    /**
     * Retourne le complément d'un ensemble de carte, c'est-à-dire l'ensemble de cartes pas contenues dans l'ensemble
     ...
     * @param that (CardSet)
     *          l'ensemble qu'on veut unir avec celui-ci
     * @return (CardSet) l'union de deux ensembles de cartes
     */
    public CardSet union(CardSet that) {
        return new CardSet(PackedCardSet.union(packedCardSet, that.packedCardSet));
    }
    
    /**
     * Retourne l'intersection de deux ensembles de cartes
     * @param that (CardSet)
     *          l'ensemble avec lequel on veut prendre l'intersection
     * @return (CardSet) l'intersection de deux ensembles de cartes 
     */
    public CardSet intersection(CardSet that) {
        return new CardSet(PackedCardSet.intersection(packedCardSet, that.packedCardSet));
    }
    
    /**
     * Calcule la différence avec un autre ensemble de cartes et retourne l'ensemble des cartes qui 
     * se trouvent dans cet ensemble mais pas dans celui donné en argument
     * @param that (CardSet)
     *          ensemble qu'on veut comparer avec celui-ci
     * @return (CardSet) l'ensemble des cartes se trouvant dans cet ensemble mais pas celui donné en argument
     */
    public CardSet difference(CardSet that) {
        return new CardSet(PackedCardSet.difference(packedCardSet, that.packedCardSet));
    }
    
    /**
     * Retourne le sous-ensemble de cet ensemble de cartes constitué uniquement des cartes de la couleur choisie
     * @param color (Card.Color)
     *          la couleur voulue
     * @return (CardSet) l'ensemble de cartes qui contient seulement les cartes de la couleur choisie
     */
     ...
     * @return (int) le résultat de la méthode hashCode de la classe Long appliquée à 
     * l'ensemble empaqueté
     */
    @Override
    public int hashCode() {
