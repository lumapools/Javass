...
public final class TurnState {
     ...
     * ou lève IllegalArgumentException si l'une d'entre elles est invalide
     * selon la méthode isValid correspondante
     * 
     * @param pkScore
     *            (long) score empaqueté
     * @param pkUnplayedCards
     *            (long) l'ensemble de cartes pas jouées
     * @param pkTrick
     *            (int) pli empaqueté
    ...
    /**
     * Accesseur permettant d'obtenir la version empaquetée de l'ensemble des
     * cartes pas encore jouées
     * 
    ...
    /**
     * Accesseur permettant d'obtenir la version objet de l'ensemble des cartes
     * pas encore jouées
     * 
     ...
     * prochain joueur ait joué la carte donnée, ou lève IllegalStateException
    ...
    public TurnState withNewCardPlayed(Card card) throws IllegalStateException {
        ...
        return new TurnState(packedScore(),
                PackedCardSet.remove(packedUnplayedCards(), pkCard),
                PackedTrick.withAddedCard(packedTrick(), pkCard));
    }

    /**
     * Retourne l'état correspondant à celui auquel on l'applique après que le
     * pli courant ait été ramassé, ou lève IllegalStateException si le pli
     * courant n'est pas terminé (c-à-d plein)
     * 
     * @return (TurnState) l'état correspondant
     * @throws IllegalStateException
    ...
    public TurnState withTrickCollected() throws IllegalStateException {
        ...
        long newCurrentScore = PackedScore.withAdditionalTrick(packedScore(),
                PackedTrick.winningPlayer(packedTrick()).team(),
                PackedTrick.points(packedTrick()));
        int newCurrentTrick = PackedTrick.nextEmpty(packedTrick());
        return new TurnState(newCurrentScore, packedUnplayedCards(),
                newCurrentTrick);
    }
     ...
     * ramassé s'il est alors plein; lève IllegalStateException si le pli
     * courant est plein
     * 
     * @param card
     *            (Card) carte à rajouter
     * @return (TurnState) l'état correspondant
     * @throws IllegalStateException
     *             si le pli courant avant d'ajouter la carte est plein
     */
