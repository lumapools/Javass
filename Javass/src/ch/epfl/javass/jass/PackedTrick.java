...
public final class PackedTrick {
	 ...
	 * 
	 * @param pkTrick (int) le pli empaqueté dans une séquence de 32 bits
	...
	public static boolean isValid(int pkTrick) {
		...
		return (PackedCard.isValid(card1) && PackedCard.isValid(card2) && PackedCard.isValid(card3)
				&& PackedCard.isValid(card4)
				|| (PackedCard.isValid(card1) && PackedCard.isValid(card2) && PackedCard.isValid(card3)
						&& card4 == PackedCard.INVALID)
				|| (PackedCard.isValid(card1) && PackedCard.isValid(card2) && card3 == PackedCard.INVALID
						&& card4 == PackedCard.INVALID)
				|| (PackedCard.isValid(card1) && card2 == PackedCard.INVALID && card3 == PackedCard.INVALID
						&& card4 == PackedCard.INVALID)
				|| (card1 == PackedCard.INVALID && card2 == PackedCard.INVALID && card3 == PackedCard.INVALID
						&& card4 == PackedCard.INVALID));
	}

	/**
	 * Retourne un pli vide qui contient juste le premier joueur voulu et la couleur
	 * d'atout voulue
	 * 
	 * @param trump       (Card.Color) la couleur d'atout voulu
	 * @param firstPlayer (PlayerId) le premier joueur voulu
	 * @return (int) le premier pli du jeu qui ne contient aucune carte, juste le
	 *         joueur qui commence le pli et l'atout
	 */
	 ...
	 * 
	 * @param pkTrick (int) le pli empaqueté qui est le pli précédent celui qu'on
	 *                veut retourner
	 * @return (int) le prochain pli empaqueté sans les cartes (ou un pli invalide
	 *         si le pli courant est le dernier)
	 */
	public static int nextEmpty(int pkTrick) {
		...
		return Bits32.pack(nextIndex, NB_BITS_FOR_INDEX, nextPlayer, NB_BITS_FOR_PLAYER, currentTrump,
				NB_BITS_FOR_TRUMP) << NB_BITS_FOR_CARDS | Bits32.mask(0, NB_BITS_FOR_CARDS);
	}
	...
	/**
	 * Retourne la version empaquetée de la carte du pli à l'index donné (supposée
	 * avoir été posée)
	 * 
	 * @param pkTrick (int) pli empaqueté
	...
	/**
	 * Retourne un pli identique à celui donné (supposé non plein), mais à laquelle
	 * la carte donnée a été ajoutée
	 * 
	 * @param pkTrick (int) pli empaqueté
	...
	public static int withAddedCard(int pkTrick, int pkCard) {
		...
		return (pkCard << shiftBeforeCard | Bits32.mask(0, shiftBeforeCard)
				| Bits32.mask(shiftAfterCard, Integer.SIZE - shiftAfterCard)) & pkTrick;
	}
	 ...
	 * peuvent être jouées comme prochaine carte du pli pkTrick (supposé non plein)
	 * 
	 * @param pkTrick (int) pli empaqueté
	 * @param pkHand  (long) les cartes dans les mains
	 ...
	 * 
	 * @param pkTrick (int) le pli courant
	...
	public static String toString(int pkTrick) {
		assert isValid(pkTrick);
		String representation = "(" + "Pli : " + index(pkTrick) + ", ";
		for (int i = 0; i < size(pkTrick); ++i) {
