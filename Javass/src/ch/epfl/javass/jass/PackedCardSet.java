...
public final class PackedCardSet {
	// Nombre de bits pour représenter une ensemble de cartes appartenant à la
	// même couleur (y compris les 7 bits inutilisés)
	private static final int COLOR_SET_SIZE = 16;

	/*
	 * Constantes qui représentent un ensemble de cartes meilleures qu'une carte
	 * donnée D'ATOUT
	 */
	...
	// tableau qui contient ces cartes dessus, utilisé pour la méthode
	// TrumpAbove
	private static final int tableBetterCards[] = new int[] { BETTER_THAN_SIX, BETTER_THAN_SEVEN, BETTER_THAN_EIGHT,
			BETTER_THAN_TEN, BETTER_THAN_QUEEN, BETTER_THAN_KING, BETTER_THAN_ACE, BETTER_THAN_NINE, BETTER_THAN_JACK };
	 ...
	 * 
	 * @param pkCardSet (long) l'ensemble de cartes
	 * @return (boolean) faux si l'ensemble de cartes est invalide (c'est-à-dire
	 *         contient un/des 1 à un/des endroits pas voulus)
	 */
	public static boolean isValid(long pkCardSet) {
		return (ALL_CARDS & pkCardSet) == pkCardSet;
	}

	/**
	 * Etant donné une carte d'atout représentée sous form de 32 bits, cette méthode
	 * retourne un long qui représente un ensemble de cartes qui contient toutes les
	 * cartes qui sont plus fortes que cette carte
	 * 
	 * @param pkCard (long) carte empaquetée représentée sous une forme de 32 bits
	 * @return (long) l'ensemble de cartes (d'atout nécéssairement) qui sont plus
	 *         fortes que pkCard
	 */
	public static long trumpAbove(int pkCard) {
		assert PackedCard.isValid(pkCard);
		return ((long) tableBetterCards[PackedCard.rank(pkCard).trumpOrdinal()]) << COLOR_SET_SIZE
				* PackedCard.color(pkCard).ordinal();
	}
	 ...
	 * empaquetée donnée
	 * 
	 * @param pkCard (int) la carte empaquetée
	 ...
	 * 
	 * @param pkCardSet (long) ensemble de carte empaqueté
	...
	/**
	 * Retourne la taille de l'ensemble de cartes empaqueté donné, c-à-d le nombre
	 * de cartes qu'il contient
	 * 
	 * @param pkCardSet (long) ensemble de cartes empaqueté
	 ...
	 * 
	 * @param pkCardSet (long) ensemble de cartes empaqueté
	 * @param index     (int) l'index dans l'ensemble
	 * @return (int) la version empaquetée de la carte
	 */
	public static int get(long pkCardSet, int index) {
		assert isValid(pkCardSet);
		assert 0 <= index && index < size(pkCardSet);
	 ...
	 * 
	 * @param pkCardSet (long) ensemble empaqueté de départ
	 * @param pkCard    (int) carte empaquetée
	 ...
	 * 
	 * @param pkCardSet (long) ensemble des cartes empaqueté
	 ...
	 * 
	 * @param pkCardSet1 (long) premier ensemble de cartes empaqueté
	 * @param pkCardSet2 (long) deuxième ensemble de cartes empaqueté
	...
	/**
	 * Calcul la différence entre le premier ensemble de cartes empaqueté donné et
	 * le second, c-à-d l'ensemble des cartes qui se trouvent dans le premier
	...
	/**
	 * Retourne le sous-ensemble de l'ensemble de cartes empaqueté donné constitué
	 * que de cartes de la couleur qu'on a choisie
	 * 
	 * @param pkCardSet (long) l'ensemble de cartes empaquetées dont on veut
	 *                  extraire une couleur spécifique
	 * @param color     (Card.Color) la couleur qu'on veut extraire
	 * @return (long) l'ensemble empaqueté de cartes dont on a choisi la couleur
	 */
	public static long subsetOfColor(long pkCardSet, Card.Color color) {
		assert isValid(pkCardSet);
		return Bits64.extract(pkCardSet, color.ordinal() * COLOR_SET_SIZE, COLOR_SET_SIZE) << color.ordinal()
				* COLOR_SET_SIZE;
	 ...
	 * 
	 * @param pkCardSet (long) la séquence de 64 bits qui représente un ensemble de
	 *                  cartes
	 * @return (String) un string des cartes contenues dans l'ensemble (comme s'il
	 *         s'agissait d'un set)
	 */
