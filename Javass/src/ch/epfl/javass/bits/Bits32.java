...
final public class Bits32 {
     ...
     * @return (int) le nombre crée par le masque 
     */
    public static int mask(int start, int size) {
        if((start == 0) && (size == Integer.SIZE)) {
            return ~0;
        }
        if(start == Integer.SIZE && size == 0) {
            return 0;
        }
        else {
            if((start < 0) || (start > Integer.SIZE) || start + size > Integer.SIZE || size < 0) {
                ...
                if(start + size == Integer.SIZE) {
                    return ((1 << start + size)-1) - (1<<start);
                }
                else {
                    return((1 << start + size)-1) - ((1<<start)-1);
                }
            }
        }
        return 0;
     ...
     * 
     * @param bits
     *          nombre dont on veut extraire une partie (int)
     * @param start
    ...
    /**
     * Construit une séquence de 32 bits en mettant bout à bout les bits donnés en arguments
     * 
     * @param v1 (int)
     *          premier nombre dans dans la séquence de 32 bits
     * @param s1 (int)
     *          la longueur voulue qui sera occupée par v1
     * @param v2 (int)
     *          vient après v1 dans la séquence de 32 bits
     * @param s2 (int)
     *          longueur voulue qui sera occupée par v2
     * @throws IllegalArgumentException si
     *          1) Si la somme s1+s2 dépasse 32
    ...
    /**
     * Construit une séquence de 32 bits en mettant bout à bout les bits donnés en arguments
     * On va dans le sens des bits de poids faible aux bits de poids fort lors de l'assemblage
     * 
     * @param v1 (int)
     *          vient en premier dans la séquence de 32 bits
     * @param s1 (int) 
     *          longueur voulue de v1
     * @param v2 (int)
     *          vient en deuxième dans la séquence de 32 bits
     * @param s2 (int)
     *          longueur voulue de v2
     * @param v3 (int)
     *          vient en troisième dans la séquence de 32 bits
     * @param s3 (int)
     *          longueur voulue de v3
     * @return (int) la séquence de 32 bits construite 
     * @throws IllegalArgumentException
     *          @throws IllegalArgumentException si
     *          1) Si la somme s1+s2+s3 dépasse 32
     *          2) Si on donne une longueur négative pour s1, s2 ou s3
     *          3) Si l'int donné en paramètre occupe plus de place (bits) que voulu en argument
     */
    ...
    /**
     * Construit une séquence de 32 bits en mettant bout à bout les bits v1 à v7.
     * On va dans le sens des bits de poids faible aux bits de poids fort lors de l'assemblage
     * 
     * @param v1 (int)
     *          vient en premier dans la séquence de 32 bits
     ...
     *          longueur voulue de v3
     * @param v4 (int)
     *          vient en quatrième dans la séquence de 32 bits
     * @param s4 (int)
     *          longueur voulue de v4
     * @param v5 (int)
     *          vient en cinquième dans la séquence de 32 bits
     * @param s5 (int)
     *          longueur voulue de v5
     * @param v6 (int)
     *          vient en sixième dans la séquence de 32 bits
     * @param s6 (int)
     *          longueur voulue de v6
     * @param v7 (int)
     *          vient en septième dans la séquence de 32 bits
     * @param s7 (int)
     *          longueur voulue de v7
     * @return (int) la séquence de 32 bits construite
     * @throws IllegalArgumentException
     *          1) Si la somme s1+s2+s3+s4+s5+s6+s7 dépasse 32
     *          2) Si on donne une longueur négative pour s1, s2, s3, s4, s5, s6 ou s7
     *          3) Si l'int donné en paramètre occupe plus de place (bits) que voulu en argument
     *              
     */
